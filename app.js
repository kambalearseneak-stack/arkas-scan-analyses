...
function updateFormMode() {
    ...
    if (isSignUpMode) {
        ...
        if (formTitle) formTitle.textContent = "Créer un compte";
        if (formSubtitle) formSubtitle.textContent =
            "Créez votre compte ARKAS SCAN AI pour accéder au scanner SMC";
        if (submitBtn && !submitBtn.disabled) submitBtn.textContent = "S'inscrire";
        if (toggleLabel) toggleLabel.textContent = "Vous avez déjà un compte ?";
        if (toggleModeBtn) toggleModeBtn.textContent = "Se connecter";
        if (passwordInput) passwordInput.setAttribute("autocomplete", "new-password");

    } else {

        if (formTitle) formTitle.textContent = "ARKAS SCAN AI";
        if (formSubtitle) formSubtitle.textContent =
            "Connectez-vous pour accéder à vos analyses SMC";
        if (submitBtn && !submitBtn.disabled) submitBtn.textContent = "Se connecter";
        if (toggleLabel) toggleLabel.textContent = "Pas encore de compte ?";
        if (toggleModeBtn) toggleModeBtn.textContent = "S'inscrire";
    ...
    loginForm.addEventListener("submit", async function (event) {
        ...
        try {
            ...
            if (isSignUpMode) {
                ...
                console.log("✅ Connexion :", userCredential.user.email);
                window.location.replace("dashboard.html");
            }
...
onAuthStateChanged(auth, function (user) {
    ...
    if (isDashboardPage) {
        ...
        if (analysisStatus) analysisStatus.textContent = "Prêt";
    }
});
